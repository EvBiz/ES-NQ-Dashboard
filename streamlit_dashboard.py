...
df = yf.Ticker('ES=F').history(period='2y')
dfnq = yf.Ticker('NQ=F').history(period='2y')
dfvix = yf.Ticker('^vix').history(period='5y')

# Calculate return and the Moving Averages
...
atr = round((true_range.rolling(14).sum()/14).iat[-1],2)
